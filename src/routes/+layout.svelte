<script lang="ts">
  ...
  let updateAvailable: {
    version: string;
    body?: string;
    downloadAndInstall: (cb: () => void) => Promise<void>;
  } | null = $state(null);
  ...
  let appWindow:
    | {
        ...
        onResized: (cb: () => Promise<void>) => void;
        toggleMaximize: () => void;
        startDragging: () => void;
        minimize: () => void;
        close: () => void;
  ...
  async function checkForUpdates(): Promise<void> {
    try {
      ...
      // Offline or error: let the user in
      showApp();
    }
  }
  ...
  async function installUpdate(): Promise<void> {
    ...
    try {
      await updateAvailable.downloadAndInstall(() => {
        // Progress tracking handled by UI
      });
  ...
  // Titlebar
  function handleTitlebarMousedown(e: MouseEvent): void {
